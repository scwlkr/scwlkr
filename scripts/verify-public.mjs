...
requireCondition([301, 308].includes(redirect.status), `HTTP apex returned ${redirect.status}, expected 301 or 308`);
const redirectLocation = redirect.headers.get("location");
requireCondition(Boolean(redirectLocation), "HTTP apex redirect is missing Location");
requireCondition(new URL(redirectLocation, httpUrl).href === new URL("/", baseUrl).href, "HTTP apex redirected elsewhere");
