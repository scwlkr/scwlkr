...
5. **Production Push:** Run `npm run build`. This generates the `/dist` folder. Ensure your root `scwlkr.com` Vercel deployment grabs this `/dist` output and routes it to the `/bubba-dashboard` path cleanly. Run `bash scripts/audit.sh` to confidently check the live URL.
